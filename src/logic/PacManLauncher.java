	...
	public void animate () {
		...
		while ((this.maps.getNbGom() > 0) && (this.pacman.getLife() > 0) && (this.nbrSamples < PacManLauncher.nbrMaxSample)) {
			...
			if(Canvas.getCanvas().isAIdriven()) {//c'est l'IA qui joue
				...
				isInit = this.pacman.move(QLearningAI.findNextMove(this.maps.getVisibleBeliefState().get(0)));//l'IA (politique RL apprise) choisit un mouvement et Pacman commence a se deplacer
